...
def generar_historico():
    ...
    rows.sort(
        key=lambda r: (
            0 if r["count"] > 1 else 1,
            0 if r["appears_in_last_top"] == "Sí" and r["estado"] == "Reincidente" else 1,
            -r["count"],
            r["first_label"],
            r["mc"],
        )
    ...
    start_row = 3
    for i, (label, value, fill) in enumerate(res_rows, start=start_row):
    ...
    top_start = 8
    ws_res.merge_cells(start_row=top_start, start_column=1, end_row=top_start, end_column=4)
    ...
    headers = ["MC", "Veces", "Ciclos", "Estado"]
    ...
    for i, r in enumerate(top_10, start=top_start + 2):
        vals = [r["mc"], r["count"], r["ciclos"], r["estado"]]
        for col, v in enumerate(vals, start=1):
            ...
            cell.font = body_font
            if r["estado"] == "Reincidente":
                cell.fill = fill_red
            elif r["estado"] == "Nuevo":
                cell.fill = fill_green
            else:
                cell.fill = fill_amber if r["count"] > 1 else fill_gray

    for col, width in {1: 22, 2: 12, 3: 55, 4: 16}.items():
    ...
    for row_idx, r in enumerate(rows, start=2):
        ...
        for col, v in enumerate(vals, start=1):
            ...
            cell.border = border_thin
            cell.alignment = Alignment(horizontal="left" if col in (1,3,4,5,6,7) else "center", vertical="center")
            cell.font = body_font
            if r["estado"] == "Reincidente":
                cell.fill = fill_red
            elif r["estado"] == "Nuevo":
                cell.fill = fill_green
            elif r["appears_in_last_top"] == "Sí":
