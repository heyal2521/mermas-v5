...
<body style="font-family:Arial,Helvetica,sans-serif;max-width:900px;margin:20px;">
  ...
  <p style="color:gray;font-size:0.9em">Sube tu histórico acumulado anterior y añade los TOP nuevos que quieras incorporar.</p>
  <form action="/historico_acumulado" method="post" enctype="multipart/form-data">
    <label>Histórico acumulado anterior: <input type="file" name="master" accept=".xlsx" required></label><br><br>
    <input type="file" name="files" multiple accept=".xlsx,.xlsm,.xls" required><br><br>
    <button type="submit">Actualizar acumulado</button>
  </form>
...
def _seed_records_from_master_workbook(master_wb):
    ...
    for r in range(2, ws.max_row + 1):
        ...
        ciclos = [c.strip() for c in str(ciclos_raw).split(",") if c.strip()]
        ...
        records[mc] = {
            ...
            "ciclos": ciclos,
            "first_idx": 0,
            "first_label": first_label or "HISTORICO ACUMULADO",
            "last_idx": 0,
            "last_label": last_label or "HISTORICO ACUMULADO",
        }

    return records


def _build_historico_rows(records, last_seen_models, last_label):
    rows = []
    ...
    return rows, total_models, new_models, repeated_models, last_label


def _render_historico_visual(rows, total_models, new_models, repeated_models, sheet_title="HISTORICO_ACUMULADO"):
    wb_out = Workbook()
...
def generar_historico_acumulado():
    ...
    if master_file and master_file.filename and master_file.filename.lower().endswith('.xlsx'):
        try:
            ...
            records = _seed_records_from_master_workbook(master_wb)
        except Exception:
            records = {}

    last_seen_models = set()
    last_label = None
    base_idx = 1 if records else 0

    for idx, f in enumerate(valid_files, start=base_idx):
        try:
            label, models = _extract_top_models_and_label(f)
            last_label = label
            for mc in models:
            ...
            if idx == (base_idx + len(valid_files) - 1):
    ...
    rows, total_models, new_models, repeated_models, last_label = _build_historico_rows(records, last_seen_models, last_label)
    output = _render_historico_visual(rows, total_models, new_models, repeated_models, sheet_title="HISTORICO_ACUMULADO")
