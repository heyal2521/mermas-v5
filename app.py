...
def generar_historico():
    ...
    for idx, f in enumerate(valid_files):
        try:
            ...
            if idx == len(valid_files) - 1:
                ultimo_fichero = etiqueta

            modelos_en_fichero = set()
            for r in range(2, ws.max_row + 1):
                ...
                if not mc:
                    continue
                modelos_en_fichero.add(mc)

            for mc in modelos_en_fichero:
                if mc not in data:
    ...
    headers = ["MC", "Veces", "Ciclos"]
    if ultimo_fichero:
        headers.append("Último fichero")
    ...
    for mc in sorted(data.keys()):
        ...
        if ultimo_fichero:
            row.append(ultimo_fichero if ultimo_fichero in info["ciclos"] else "")
        ws_out.append(row)
