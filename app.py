...
def _sequence_stats(order, labels):
    ...
    for label in labels:
        if label in order_map:
            positions.append(order_map[label])
...
def _render_historico_visual(rows, total_models, new_models, repeated_models, persistent_total_count, persistent_consecutive_count, family_rows, alerts, order, sheet_title="HISTORICO"):
    ...
    card_start = 3
    for i, (label, value, fill) in enumerate(summary_cards, start=card_start):
    ...
    for row_idx, r in enumerate(rows, start=2):
        vals = [
            ...
            r["prioridad"],
            r["family"],
            r["persistencia_total"],
            r["persistencia_consecutiva"],
            r["racha_max"],
        ]
        for col, v in enumerate(vals, start=1):
    ...
    widths = {
        1: 18, 2: 10, 3: 44, 4: 18, 5: 18, 6: 24,
        7: 16, 8: 14, 9: 18, 10: 20, 11: 20, 12: 12
    }
...
def generar_historico():
    ...
    output = _render_historico_visual(rows, total_models, new_models, repeated_models, persistent_total_count, persistent_consecutive_count, family_rows, alerts, order, sheet_title="HISTORICO")
