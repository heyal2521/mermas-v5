...
def generar_historico():
    ...
    for mc, rec in records.items():
        ...
        rows.append({
            ...
            "is_new": rec["first_label"] == last_label if last_label else False,
        })

    total_models = len(rows)
    new_models = sum(1 for r in rows if r["is_new"])
    repeated_models = sum(1 for r in rows if r["count"] > 1)

    rows.sort(
        key=lambda r: (
            0 if r["count"] > 1 else 1,
            0 if r["appears_in_last_top"] == "Sí" and not r["is_new"] else 1,
            -r["count"],
            r["first_label"],
            r["mc"],
        )
    ...
    ws_res["A3"] = "Total de modelos"
    ws_res["B3"] = total_models
    ws_res["A4"] = "Cuántos son nuevos"
    ws_res["B4"] = new_models
    ws_res["A5"] = "Cuántos se repiten"
    ws_res["B5"] = repeated_models
    ws_res["A7"] = "Top 10 reincidentes"
    ws_res["A8"] = "MC"
    ws_res["B8"] = "Veces"
    ws_res["C8"] = "Ciclos"

    top_10 = sorted([r for r in rows if r["count"] > 1], key=lambda x: (-x["count"], x["mc"]))[:10]
    for i, r in enumerate(top_10, start=9):
        ws_res.cell(row=i, column=1).value = r["mc"]
        ws_res.cell(row=i, column=2).value = r["count"]
        ws_res.cell(row=i, column=3).value = r["ciclos"]
